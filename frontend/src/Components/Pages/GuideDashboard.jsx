        ...
        <div className="min-h-screen bg-aurora pt-24 pb-20">
            <div className="max-w-7xl mx-auto px-6">
                ...
                <motion.div
                    ...
                    className="glass-panel p-8 md:p-12 rounded-[40px] border-azure/20 mb-10 relative overflow-hidden"
                >
                    <div className="absolute top-0 right-0 p-10 opacity-5">
                        <Shield className="w-40 h-40 text-azure" />
                    </div>
                    <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
                        <div className="w-24 h-24 rounded-3xl bg-gradient-to-tr from-azure to-aurora flex items-center justify-center text-3xl font-bold text-white shadow-lg overflow-hidden border-2 border-azure/20">
                        ...
                        <div className="text-center md:text-left">
                            <h1 className="text-3xl md:text-4xl font-extrabold text-white mb-2 underline decoration-azure decoration-4 underline-offset-8">Guide Command Center</h1>
                ...
                <div className="space-y-6">
                    ...
                    {loading ? (
                        ...
                        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid gap-4">
                            {bookings.map((b) => (
                                <motion.div key={b.id} variants={itemVariants} className="glass-panel p-6 rounded-3xl flex flex-col lg:flex-row items-center justify-between border-white/5 hover:border-azure/20 transition-all group">
                                    <div className="flex items-center gap-6 w-full">
                                        <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center font-bold text-azure border border-azure/20">
                                            <User className="w-5 h-5" />
                                        </div>
                                        <div className="flex-1">
                                            <div className="flex items-center gap-3 mb-1">
                                                <h3 className="text-lg font-bold text-white">{b.traveler_name}</h3>
                                                <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-widest ${b.status === 'confirmed' ? 'bg-green-500/10 text-green-400' :
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs text-gray-500 font-medium">
                                                ...
                                                <span className="flex items-center gap-1"><Phone className="w-3 h-3 text-azure" />{b.contact_phone}</span>
                                            </div>
                                        </div>
                                    </div>

                                    <div className="flex items-center gap-3 mt-6 lg:mt-0 w-full lg:w-auto justify-end">
